    ...
    else if (dataState == STATE_MSGTYPE)
    {
        ...
        { // received checksum byte
            ...
            { // wrong checksum, drop packet
                dataState = IDLE;
                crsf_tlm_data.telemetry_failed_cs++;
            }
        }
        else
        {
...
bool parseLTM_GFRAME(uint32_t now)
{
    ...
    {
        crsf_tlm_data.telemetry_lat = (int32_t)ltmread_u32();
        crsf_tlm_data.telemetry_lat = crsf_tlm_data.telemetry_lat / 10;
        crsf_tlm_data.telemetry_lon = (int32_t)ltmread_u32();
        crsf_tlm_data.telemetry_lon = crsf_tlm_data.telemetry_lon / 10;
        crsf_tlm_data.telemetry_speed = ltmread_u8();
        crsf_tlm_data.telemetry_alt = (int32_t)ltmread_u32(); // 10000000;
        crsf_tlm_data.telemetry_alt = (int16_t)(crsf_tlm_data.telemetry_alt / 100);
        satsfix = ltmread_u8();
        crsf_tlm_data.telemetry_sats = (int16_t)((satsfix >> 2) & 0xFF);
        crsf_tlm_data.gps_fix_type = satsfix & 0b00000011;
        if (crsf_tlm_data.telemetry_sats >= 5)
        {
            crsf_tlm_data.telemetry_gotFix = 1;
        }
        crsf_tlm_data.telemetry_gotAlt = true;
        result = true;
        // DBGLN("LTM_GFRAME");
    }
    else if (LTM_cmd == LTM_AFRAME)
    {
        crsf_tlm_data.telemetry_ltm_pitch = radians((int16_t)ltmread_u16());
        crsf_tlm_data.telemetry_ltm_roll = radians((int16_t)ltmread_u16());
        crsf_tlm_data.telemetry_course = (int16_t)ltmread_u16() * 1.0f;
        crsf_tlm_data.telemetry_ltm_yaw = radians(crsf_tlm_data.telemetry_course);
        result = true;
        DBGLN("LTM_AFRAME %.2f %.2f", crsf_tlm_data.telemetry_pitch, crsf_tlm_data.telemetry_roll);
    }
    dataState = IDLE;
